<footer class="bg-black text-white pt-32 pb-12 overflow-hidden relative border-t border-gray-900">
  <!-- CTA Section -->
  <div class="max-w-7xl mx-auto px-6 mb-32 relative z-10">
    <div class="flex flex-col items-center text-center">
      <!-- Logo -->
      <div class="mb-12 relative w-24 h-24 flex items-center justify-center">
        <!-- Grid decoration -->
        <div class="absolute inset-0 border border-white/10"></div>
        <div class="absolute inset-4 border border-white/10"></div>
        
        <!-- Stables Icon -->
        <svg width="48" height="48" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="animate-pulse">
          <rect x="3" y="3" width="8" height="8" rx="1" fill="white"/>
          <rect x="13" y="3" width="8" height="8" rx="1" fill="white"/>
          <rect x="3" y="13" width="8" height="8" rx="1" fill="white"/>
          <rect x="13" y="13" width="8" height="8" rx="1" fill="white"/>
        </svg>
      </div>

      <h2 class="text-4xl md:text-5xl font-medium tracking-tight mb-12 max-w-2xl">
        Ready to unlock your crypto wealth?
      </h2>

      <div class="flex flex-col sm:flex-row items-center gap-4 w-full max-w-md border-b border-white/20 pb-2">
        <input 
          type="email" 
          placeholder="Enter your email" 
          class="bg-transparent border-none outline-none text-white placeholder-gray-500 w-full text-lg py-2"
        />
        <button class="text-primary-400 hover:text-primary-300 whitespace-nowrap font-medium flex items-center gap-2 transition-colors">
          Get Started <span>→</span>
        </button>
      </div>
    </div>
  </div>

  <!-- Background Grid Decoration -->
  <div class="absolute top-0 left-0 right-0 h-[600px] pointer-events-none opacity-20" 
       style="background-image: linear-gradient(rgba(255,255,255,0.1) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.1) 1px, transparent 1px); background-size: 40px 40px;">
  </div>

  <!-- Footer Links -->
  <div class="max-w-7xl mx-auto px-6 relative z-10">
    <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-12 mb-32 text-sm">
      <div class="space-y-4">
        <h4 class="text-gray-500 font-medium">Product</h4>
        <ul class="space-y-3">
          <li><a href="#" class="hover:text-gray-300">Features</a></li>
          <li><a href="#" class="hover:text-gray-300">Supported Chains</a></li>
          <li><a href="#" class="hover:text-gray-300">Countries</a></li>
          <li><a href="#" class="hover:text-gray-300">Pricing</a></li>
        </ul>
      </div>
      <div class="space-y-4">
        <h4 class="text-gray-500 font-medium">Developers</h4>
        <ul class="space-y-3">
          <li><a href="#" class="hover:text-gray-300">API Documentation</a></li>
          <li><a href="#" class="hover:text-gray-300">Integration Guide</a></li>
          <li><a href="#" class="hover:text-gray-300">SDKs</a></li>
          <li><a href="#" class="hover:text-gray-300">API Status</a></li>
        </ul>
      </div>
      <div class="space-y-4">
        <h4 class="text-gray-500 font-medium">Resources</h4>
        <ul class="space-y-3">
          <li><a href="#" class="hover:text-gray-300">Blog</a></li>
          <li><a href="#" class="hover:text-gray-300">Help Center</a></li>
          <li><a href="#" class="hover:text-gray-300">FAQ</a></li>
          <li><a href="#" class="hover:text-gray-300">Support</a></li>
        </ul>
      </div>
      <div class="space-y-4">
        <h4 class="text-gray-500 font-medium">Company</h4>
        <ul class="space-y-3">
          <li><a href="#" class="hover:text-gray-300">About Us</a></li>
          <li><a href="#" class="hover:text-gray-300">Team</a></li>
          <li><a href="#" class="hover:text-gray-300">Careers</a></li>
          <li><a href="#" class="hover:text-gray-300">Contact</a></li>
        </ul>
      </div>
      <div class="space-y-4">
        <h4 class="text-gray-500 font-medium">Legal</h4>
        <ul class="space-y-3">
          <li><a href="#" class="hover:text-gray-300">Terms of Service</a></li>
          <li><a href="#" class="hover:text-gray-300">Privacy Policy</a></li>
          <li><a href="#" class="hover:text-gray-300">Cookie Policy</a></li>
          <li><a href="#" class="hover:text-gray-300">Compliance</a></li>
        </ul>
      </div>
    </div>

    <!-- Big Logo -->
    <div class="mb-12">
      <h1 class="text-[12vw] leading-none font-bold tracking-tighter text-white opacity-90 select-none">Stables</h1>
    </div>

    <!-- Bottom Bar -->
    <div class="flex flex-col md:flex-row justify-between items-center gap-6 text-xs text-gray-500 border-t border-white/10 pt-8">
      <div class="flex gap-6">
        <a href="#" class="hover:text-white">Twitter</a>
        <a href="#" class="hover:text-white">Discord</a>
        <a href="#" class="hover:text-white">Telegram</a>
        <a href="#" class="hover:text-white">GitHub</a>
      </div>
      <div>
        © 2025 Stables. All rights reserved.
      </div>
    </div>
  </div>
</footer>
